...
end_match = re.search(r'function updateTTSButtons\(\) \{', content)
...
if not start_match or not end_match:
    ...
    if end_idx < start_idx:
        print("CRITICAL ERROR: end_idx is before start_idx. Aborting.")
    else:
        new_block = """        async function speakNarration(text, syncElements = []) {
            ...
            if (syncElements && syncElements.length > 0) {
                ...
                sentences.forEach((sen) => {
                    ...
                    if (sen.trim().length > 1 && /[a-zA-Z0-9]/.test(sen)) {
...
"""
        ...
        # Write back to original encoding
        try:
